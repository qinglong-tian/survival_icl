...
# Fir two-H100 launcher for the canonical survival Stage 1 curriculum.
#
...
#       scripts/train_survival_stage1_fir.sh
#
...
VENV_PATH="${VENV_PATH:-${FIR_VENV_DIR}/bin/activate}"

case "$RUN_MODE" in
    test)
        STAGE1_TARGET_STEPS="${STAGE1_TARGET_STEPS:-50}"
        STAGE1_CHUNK_STEPS="${STAGE1_CHUNK_STEPS:-50}"
        CURRICULUM_ID="${CURRICULUM_ID:-fir_stage1_test_${JOB_ID}}"
        SURVIVAL_CHECKPOINT_DIR="${SURVIVAL_CHECKPOINT_DIR:-/scratch/${USER}/survival-icl-fir-tests/${JOB_ID}}"
        AUTO_RESUBMIT=0
        ;;
    formal)
        STAGE1_TARGET_STEPS="${STAGE1_TARGET_STEPS:-100000}"
        STAGE1_CHUNK_STEPS="${STAGE1_CHUNK_STEPS:-500}"
        CURRICULUM_ID="${CURRICULUM_ID:-author_adapted_v1_fir}"
        SURVIVAL_CHECKPOINT_DIR="${SURVIVAL_CHECKPOINT_DIR:-/scratch/${USER}/survival-icl-fir}"
        AUTO_RESUBMIT=1
        ;;
    *)
...
CPU_COUNT="${SLURM_CPUS_PER_TASK:-12}"
...
if [[ ! -f "$FIR_VENV_DIR/.fir-env-ready" || ! -f "$VENV_PATH" ]]; then
    echo "ERROR: Fir Python environment is not ready: ${FIR_VENV_DIR}" >&2
...
python scripts/check_fir_runtime.py --expected-gpus "$GPU_COUNT"
torchrun --standalone --nproc_per_node="$GPU_COUNT" \
    scripts/check_fir_runtime.py --expected-gpus "$GPU_COUNT" --distributed
python -c "
import hashlib
