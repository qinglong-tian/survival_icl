"""Validate CUDA execution and optional single-node NCCL communication."""
...
def _initialize_driver() -> None:
    try:
        ...
        raise RuntimeError("Could not load the host CUDA driver library libcuda.so.1.") from exc
...
def _check_local_cuda(expected_gpus: int) -> None:
    _initialize_driver()
    torch.cuda.init()
...
def _check_distributed_cuda(expected_gpus: int) -> None:
...
def main() -> None:
    ...
    if args.distributed:
        _check_distributed_cuda(args.expected_gpus)
    else:
        _check_local_cuda(args.expected_gpus)
