...
# Three TabICL-style stages with progressive unfreezing.
#
# Usage:
...
#
# Freezing scheme:
#   Stage 1: freeze ColEmbedding + RowInteraction  → train ICL + survival head
#   Stage 2: freeze ColEmbedding + RowInteraction  → train ICL + survival head
#   Stage 3: unfreeze all                           → full fine-tune
#
# Pre-requisites:
...
if (( _run_1 )); then
    ...
    guard_stale_checkpoints 1 "${CHECKPOINT_DIR}/survival_mix_stage1" "${STAGE1_STEPS}"
fi
if (( _run_2 )); then
    ...
    guard_stale_checkpoints 2 "${CHECKPOINT_DIR}/survival_mix_stage2" "${STAGE2_STEPS}"
fi
if (( _run_3 )); then
    ...
    guard_stale_checkpoints 3 "${CHECKPOINT_DIR}/survival_mix_stage3" "${STAGE3_STEPS}"
fi
...
##############################################################################
# Stage 1 — small fixed-length, freeze embeddings, train ICL + survival head
##############################################################################
if (( _run_1 )); then
    run_stage "Stage1" "${CHECKPOINT_DIR}/survival_mix_stage1" "${STAGE1_STEPS}" "" \
        --lr 1e-4 \
        --batch_size_per_gp 4 \
        --micro_batch_size 4 \
        --max_seq_len 1024 \
...
##############################################################################
# Stage 2 — medium variable-length, freeze embeddings, load Stage 1
##############################################################################
if (( _run_2 )); then
    if (( _run_1 )); then
        # Stage 1 was just run — chain from its requested checkpoint.
        STAGE1_CKPT="${CHECKPOINT_DIR}/survival_mix_stage1/step-${STAGE1_STEPS}.ckpt"
        ...
        STAGE1_CKPT=$(ls -1 "${CHECKPOINT_DIR}/survival_mix_stage1"/step-*.ckpt 2>/dev/null | sort -V | tail -1 || true)
        if [[ -z "${STAGE1_CKPT:-}" ]]; then
            echo "ERROR: No Stage 1 checkpoint found." >&2; exit 1
        fi
    fi
    echo "Chaining from Stage 1 checkpoint: $(basename "$STAGE1_CKPT")"
    run_stage "Stage2" "${CHECKPOINT_DIR}/survival_mix_stage2" "${STAGE2_STEPS}" \
        "${STAGE1_CKPT}" \
        --lr 1e-5 \
        --batch_size_per_gp 2 \
        --micro_batch_size 2 \
...
##############################################################################
# Stage 3 — large variable-length, unfreeze all, full fine-tune, load Stage 2
##############################################################################
if (( _run_3 )); then
    if (( _run_2 )); then
        # Stage 2 was just run — chain from its requested checkpoint.
        STAGE2_CKPT="${CHECKPOINT_DIR}/survival_mix_stage2/step-${STAGE2_STEPS}.ckpt"
        ...
        STAGE2_CKPT=$(ls -1 "${CHECKPOINT_DIR}/survival_mix_stage2"/step-*.ckpt 2>/dev/null | sort -V | tail -1 || true)
    ...
    run_stage "Stage3" "${CHECKPOINT_DIR}/survival_mix_stage3" "${STAGE3_STEPS}" \
        "${STAGE2_CKPT}" \
        --lr 1e-5 \
        --batch_size_per_gp 1 \
        --micro_batch_size 1 \
